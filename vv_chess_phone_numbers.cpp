  ...
  for (int row = 0; row < num_rows; row++) {
    for (int col = 0; col < num_cols; col++) {
      ...
      if (c >= '0' && c <= '9') {
        ...
        if (g_trace)
          cout << "Found digit " << char_as_int << " at position " << row << ","
               << col << endl;
...
static int CalculateNumValidPhoneNumbers(
    ...
    const vector<int> &valid_starting_digits, int num_board_rows,
    int num_board_cols, int phone_number_length) {
  ...
  for (int row_as_num_length = 1; row_as_num_length < phone_number_length;
       row_as_num_length++) {
    ...
    for (auto &digit_to_digits : valid_moves_from_digit_to_digits) {
      ...
      for (auto &to_digit : to_digits) {
        num_phones_ending_in[to_digit] +=
            one_shorter_phones_ending_in[from_digit];
      }
    }
  }

  int sum = 0;
  for (auto &n : num_phones_ending_in) sum += n;
...
static vector<int> ConsumeIntsFromStdin(const string &prompt) {
  ...
  stringstream ss(tmp);
  int ti;
  while (ss >> ti) result.push_back(ti);
  return result;
}
...
static vector<char> ConsumeCharsFromStdin(const string &prompt) {
  ...
  stringstream ss(tmp);
  char tc;
  while (ss >> tc) result.push_back(tc);
...
static bool IsMoveValidForFigure(map<int, pair<int, int>> &digit_to_position,
  ...
  if (from_d == to_d) return false;

  // no such digit
  if (digit_to_position.find(from_d) == digit_to_position.end()) return false;

  if (digit_to_position.find(to_d) == digit_to_position.end()) return false;
...
static map<int, vector<int>> GetValidMoves(vector<vector<char>> &board,
                                           const string &figure,
                                           int num_board_rows,
                                           int num_board_cols) {
  ...
  vector<int> digits;
  for (auto &kv : digit_to_position) digits.push_back(kv.first);
  ...
  for (int &from_d : digits) {
    for (int &to_d : digits) {
      ...
      if (IsMoveValidForFigure(digit_to_position, figure, from_d, to_d)) {
        if (g_trace)
          cout << "Yes, move from " << from_d << " to " << to_d
               << " is valid for " << figure << endl;
        valid_moves_from_digit_to_digits[from_d].push_back(to_d);
        // make use of symmetry
        valid_moves_from_digit_to_digits[to_d].push_back(from_d);
      } else {
        if (g_trace)
          cout << "No, move from " << from_d << " to " << to_d
               << " is NOT valid for " << figure << endl;
      }
    }
  }
...
int main(int argc, char **argv) {
  ...
  int num_valid_phone_numbers = CalculateNumValidPhoneNumbers(
      board, valid_moves_from_digit_to_digits, valid_starting_digits,
